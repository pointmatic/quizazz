...
## [1.4.0] — Phase N: `@lucide/svelte` Migration

Migrate the icon dependency from the deprecated `lucide-svelte` package to the scoped `@lucide/svelte@1.16.0` package, which is the Svelte-5-native v1.x line and the only one Lucide will ship going forward. Lockstep bump applies (npm + Python). No public-API change for `<QuizBlock>` consumers — Lucide is bundled into `dist/embed/index.js` by `@sveltejs/package`, not exposed on the embed surface. `MANIFEST_SCHEMA_VERSION` unchanged (`1.0`).

- **N.b** — `app/package.json` `dependencies` swapped: `lucide-svelte ^0.563.0` → `@lucide/svelte ^1.16.0`. All 8 import sites under `app/src/lib/components/` (`AnsweredQuestionsView`, `ConfigView`, `ManifestUpload`, `NavigationTree`, `QuizChooser`, `QuizView`, `ReviewView`, `SummaryView`) rewritten to the per-icon import form (`@lucide/svelte/icons/<icon-name>`) Lucide recommends for faster cold builds. None of the 14 brand icons removed in Lucide v1 are used in this codebase, so no icon substitutions were needed. `pnpm-lock.yaml` regenerated. Local preflight: 215/215 vitest, 0/0 svelte-check, `pnpm package` + `publint` clean, 159/159 Python tests. Lockstep version bump to `1.4.0` in [`python/pyproject.toml`](python/pyproject.toml), [`python/src/quizazz/__init__.py`](python/src/quizazz/__init__.py), and [`app/package.json`](app/package.json).
