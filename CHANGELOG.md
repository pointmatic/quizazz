...
### Phase M Subplan: sql.js / WASM / IndexedDB Robustness (planned, ships as v1.3.0)

Stories M.g – M.l, planned. Audit-driven robustness work addressing the four gotchas and five patterns from [`docs/specs/sql-js-wasm-robustness.md`](docs/specs/sql-js-wasm-robustness.md). See [`docs/specs/phase-m-subplan-sql-js-robustness.md`](docs/specs/phase-m-subplan-sql-js-robustness.md) for the full plan.

- **M.g** *(landed, awaiting v1.3.0 release)* — `WasmAssetMissingError` typed error class + HEAD-fetch precheck before `initSqlJs`. New [`app/src/lib/db/errors.ts`](app/src/lib/db/errors.ts); `WASM_ASSET_URL` constant and `assertWasmAssetAvailable` exported from [`$lib/db`](app/src/lib/db/index.ts); `initDatabase` runs the precheck before `initSqlJs` so a 404 or network failure throws a typed, programmatically-catchable error instead of silently producing a half-initialised `Database`.
- **M.h** *(landed, awaiting v1.3.0 release)* — Init memoization (Pattern B). Two-layer memo in [`app/src/lib/db/database.ts`](app/src/lib/db/database.ts): module-level `sqlJsInitPromise` for the shared sql.js init (precheck + `initSqlJs`), and a per-quiz `Map<string, Promise<Database>>` for the full open sequence. Both layers clear their slot on rejection so a retry re-runs without "poison the cache" failure. Test-only `__resetMemoization` export so each test gets a clean slate.
- **M.i** — Vite asset-import WASM bundling for both UC-1/UC-2 and UC-3; eliminate checked-in `app/static/sql-wasm.wasm`; remove host-side WASM-copy step from the embed setup contract.
- **M.j** — `<QuizBlock>` `onerror` callback prop + `CustomEvent('error')` channel + internal fallback aside; `+page.svelte` layout-level `RecordingPausedBanner` driven by a `dbInit` store with status-aware actions.
- **M.k** — Repo-boundary swallow rule for typed init errors in [`scores.ts`](app/src/lib/db/scores.ts) and the `persistDatabase` site in [`lifecycle.ts`](app/src/lib/engine/lifecycle.ts).
- **M.l** — v1.3.0 lockstep release (`pyproject.toml`, `__init__.py`, `app/package.json`); push `v1.3.0` and `npm-v1.3.0` tags.
