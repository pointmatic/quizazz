...
- [x] Push tag *(developer-initiated — irreversible publish)*
  - [x] `git tag -a npm-v1.3.1 -m "@pointmatic/quizazz 1.3.1" && git push origin npm-v1.3.1` — triggers [`publish-npm.yml`](../../.github/workflows/publish-npm.yml)
- [x] Verify post-publish
  - [x] `pnpm view @pointmatic/quizazz version` returns `1.3.1`
  - [x] Provenance signature visible on the npm web UI
